...
function rouletteSelect(population = [], scored = [], rng, direction) {
  if (!Array.isArray(population) || population.length === 0) return undefined;
  const scores = Array.isArray(scored) ? scored : [];
  if (scores.length !== population.length) return population[Math.floor(rng() * population.length)];

  const eps = 1e-9;
  if (direction === 'min') {
    const maxScore = Math.max(...scores.map((s) => Number(s) || 0));
    const weights = scores.map((s) => 1 / (eps + Math.max(eps, (Number(s) || 0) - maxScore) * -1));
    const total = weights.reduce((a, b) => a + b, 0);
    if (!Number.isFinite(total) || total <= 0) return population[Math.floor(rng() * population.length)];
    let r = rng() * total;
  ...
  const minScore = Math.min(...scores.map((s) => Number(s) || 0));
  const weights = scores.map((s) => (Number(s) || 0) - minScore + eps);
  const total = weights.reduce((a, b) => a + b, 0);
  if (!Number.isFinite(total) || total <= 0) return population[Math.floor(rng() * population.length)];
  let r = rng() * total;
  for (let i = 0; i < weights.length; i += 1) {
...
function rankSelect(population = [], scored = [], rng, comparator) {
  if (!Array.isArray(population) || population.length === 0) return undefined;
  const scores = Array.isArray(scored) ? scored : [];
  if (scores.length !== population.length) return population[Math.floor(rng() * population.length)];
  ...
  for (let i = 0; i < n; i += 1) {
    const w = n - i;
    r -= w;
    if (r <= 0) return ranked[i].individual;
  }
  return ranked[n - 1].individual;
}
...
function scorePopulation(population, fitnessFn, fitnessBatchFn) {
  if (typeof fitnessBatchFn === 'function') {
    const scores = fitnessBatchFn(population);
    if (!Array.isArray(scores) || scores.length !== population.length) {
      throw new Error('GA: fitnessBatch must return an array of scores with same length as population');
    }
    for (let i = 0; i < population.length; i += 1) population[i].__ga_score = scores[i];
    return scores;
  }
...
function defaultStopFn(state, config) {
  ...
  if (stagnation > 0 && state.stagnation >= stagnation) return { stop: true, reason: 'stagnation' };
  return { stop: false };
}

function normalizeComparator(direction) {
  const dir = direction === 'min' ? 'min' : 'max';
  if (dir === 'min') return (a, b) => (a < b ? -1 : a > b ? 1 : 0);
...
export function runGeneticAlgorithm(options = {}) {
  const {
    ...
    mutate,
    stopCondition,
    returnFinalPopulation = false
  } = options;

  if (typeof createIndividual !== 'function') throw new Error('GA: createIndividual must be a function');
  if (typeof fitness !== 'function') throw new Error('GA: fitness must be a function');
  if (typeof cloneIndividual !== 'function') throw new Error('GA: cloneIndividual must be a function');
  if (typeof crossover !== 'function') throw new Error('GA: crossover must be a function');
  if (typeof mutate !== 'function') throw new Error('GA: mutate must be a function');
  ...
  const selection = typeof selectionMethod === 'string' ? selectionMethod.trim().toLowerCase() : 'tournament';
  ...
  for (let generation = 1; generation <= clampInt(maxGenerations, 1, 100000); generation += 1) {
    const scores = scorePopulation(population, fitness, fitnessBatch);
    const ranked = population
      .map((individual, idx) => ({ individual, score: scores[idx] }))
      .sort((a, b) => comparator(a.score, b.score));
    ...
    history.push({ generation, best_score: bestScore, generation_best_score: genBest.score, stagnation });

    const stopFn = typeof stopCondition === 'function' ? stopCondition : defaultStopFn;
    const stop = stopFn({ generation, bestScore, stagnation, history }, { maxGenerations, stagnationGenerations });
    if (stop?.stop) {
      return {
        best,
        ...
        history,
        ...(returnFinalPopulation
          ? {
              final_population: ranked.map((r) => cloneIndividual(r.individual)),
              final_scores: ranked.map((r) => r.score)
            }
          : null)
      };
    }
    ...
    const next = [];
    for (let i = 0; i < elite; i += 1) next.push(cloneIndividual(ranked[i].individual));

    while (next.length < popSize) {
      const pickParent = () => {
        if (selection === 'roulette') return rouletteSelect(population, scores, rng, direction);
        if (selection === 'rank') return rankSelect(population, scores, rng, comparator);
        if (selection === 'elite') return tournamentSelect(population, scores, rng, Math.max(2, elite || 2), comparator);
        return twoRoundTournament(population, scores, rng, tournamentSize, comparator);
      };
...
export const GA_UTILS = { makeRng, pickOne, clampInt };
