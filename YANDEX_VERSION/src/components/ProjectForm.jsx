...
import { processLLMData } from '../services/llmService'
...
const ProjectForm = () => {
  ...
  const formatDataForExport = () => {
    const projectData = {
      ...
      ontology: {
        // Онтология будет сгенерирована автоматически LLM на основе данных проекта и конкурентов
      },
      evolution_settings: {
    ...
    // Если загружены данные конкурентов, объединяем их
    if (competitorsData) {
      return {
  ...
  const handleSendToLLM = async () => {
    ...
    addToast('Отправка тестового запроса...', 'info')

    try {
      // Простой тестовый запрос
      const result = await processLLMData()

      setIsProcessing(false)

      if (result.success) {
        console.log('Ответ от нейросети:', result.data.text)
        addToast(`Ответ получен: ${result.data.text}`, 'success')
      } else {
        addToast(`Ошибка: ${result.error}`, 'error')
      }
  ...
  return (
    <>
      ...
      <form className="project-form">
        {/* Загрузка данных конкурентов */}
        <div className="quick-actions" style={{ marginBottom: '24px' }}>
          <div className="actions-group" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: '12px' }}>
            <span className="actions-label">Загрузка данных конкурентов:</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '100%', flexWrap: 'wrap' }}>
              <label className="action-btn" style={{ cursor: 'pointer', margin: 0, display: 'inline-block' }}>
                <input
                  type="file"
                  accept=".json"
                  onChange={handleCompetitorsFileUpload}
                  style={{ display: 'none' }}
                />
                <span style={{ position: 'relative', zIndex: 2 }}>Загрузить спаршенных данных конкурентов</span>
              </label>
              {competitorsData && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1 }}>
                  ...
                  <button
                    ...
                    onClick={handleRemoveCompetitorsData}
